module.exports = function(app){
    ...
    app.route('/checke1').post(user.checkEmail1);//05. 이메일 체크 후 비밀번호 링크 쏘기
    app.route('/checke2').post(user.checkEmail2);//05. 이메일 체크 후 비밀번호 링크 쏘기

    //app.route('/signin2').post(user.signIn2);//가입 후 닉네임 카테고리 설정
    app.route('/signin3').post(user.signIn3);//초기로그인이라고 가정
    ...
    app.get('/notice',jwtMiddleware, user.noticeList);//공지사항 리스트 조회
    app.get('/notice/info',jwtMiddleware, user.noticeInfo);//공지사항 상세 조회
